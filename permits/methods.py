...
def can_via(resource,method,access_level,permissions):
    # Accepts the permissions portion of a parent, which must be specified in the calling function.
    pcode = permissions['fields'].get(resource)
    modifier = {
        'read':4,
        'write':2,
        'execute':1
...
def isolated_permissions_of(resource, user_type):
    ...
    """
    return [(k, permissions_digit_for(user_type, v)) for k,v in PERMISSIONS[resource]['fields'].iteritems()]

def scope_allows_collection(resource, user_type, permissions):
    """ Given the resource whose collection we want to see, the user type, and the permissions fragment
    of the enclosing scope, what methods are allowed? Returns a list.
